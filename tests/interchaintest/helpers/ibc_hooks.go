...
func GetIBCHooksUserAddress(t *testing.T, ctx context.Context, chain *cosmos.CosmosChain, channel, uaddr string) string {
	// centaurid q ibchooks wasm-sender channel-0 "centaurid1hj5fveer5cjtn4wd6wstzugjfdxzl0xps73ftl" --node http://localhost:26657
	cmd := []string{"centaurid", "query", "ibchooks", "wasm-sender", channel, uaddr,
