...
centaurid init $MONIKER --chain-id $CHAINID --consensus-key-type bls12381
...
centaurid gentx $KEY 100000000000000000stake --keyring-backend $KEYRING --chain-id $CHAINID --consensus-key-type bls12381
...
cat $HOME/.banksy/config/genesis.json | jq '.consensus_params["validator"]["pub_key_types"]=["ed25519", "bls12381"]' > $HOME/.banksy/config/tmp_genesis.json && mv $HOME/.banksy/config/tmp_genesis.json $HOME/.banksy/config/genesis.json

# Start the node (remove the --pruning=nothing flag if historical queries are not needed)
centaurid start --pruning=nothing  