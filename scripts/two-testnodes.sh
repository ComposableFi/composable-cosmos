...
if [ "$CONTINUE" == "true" ]; then
    picad start --log_level trace --log_format json --home $CHAIN_DIR/$CHAINID_1 --pruning=nothing --grpc.address="0.0.0.0:$GRPCPORT_1" --grpc-web.address="0.0.0.0:$GRPCWEB_1" > $CHAIN_DIR/$CHAINID_1.log 2>&1 &
    exit 0
fi

#Kill any used ports
PORTS=(16656 26656 16657 26657 1316 1317 8080 8081 8090 9090 8091 9091)
...
for PORT_NUMBER in "${PORTS[@]}"; do
    ...
    if [ -n "$PROCESS_ID" ]; then
        echo "Process found on port $PORT_NUMBER with PID: $PROCESS_ID. Killing..."
        kill $PROCESS_ID
        echo "Process killed."
    else
...
$BINARY add-genesis-account $VAL1_ADDR "100000000000000000000000000${DENOM}" --home $CHAIN_DIR/$CHAINID_1
$BINARY add-genesis-account $VAL2_ADDR "100000000000000000000000000${DENOM}" --home $CHAIN_DIR/$CHAINID_2
$BINARY add-genesis-account $WALLET1_ADDR "100000000000000000000000000${DENOM}" --home $CHAIN_DIR/$CHAINID_1
$BINARY add-genesis-account $WALLET2_ADDR "100000000000000000000000000${DENOM}" --home $CHAIN_DIR/$CHAINID_2
$BINARY add-genesis-account $RLY1_ADDR "100000000000000000000000000${DENOM}" --home $CHAIN_DIR/$CHAINID_1
$BINARY add-genesis-account $RLY2_ADDR "100000000000000000000000000${DENOM}" --home $CHAIN_DIR/$CHAINID_2
