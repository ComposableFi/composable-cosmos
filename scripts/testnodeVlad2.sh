...
sed -i'' -e 's/max_body_bytes = /max_body_bytes = 1000/g' ~/.banksy2/config/config.toml
sed -i'' -e 's/max_tx_bytes = /max_tx_bytes = 1000/g' ~/.banksy2/config/config.toml
cat $HOME/.banksy2/config/genesis.json | jq '.app_state["gov"]["params"]["voting_period"]="45s"' > $HOME/.banksy2/config/tmp_genesis.json && mv $HOME/.banksy2/config/tmp_genesis.json $HOME/.banksy2/config/genesis.json

# Start the node (remove the --pruning=nothing flag if historical queries are not needed)
centaurid start --pruning=nothing  --minimum-gas-prices=0stake --rpc.laddr tcp://127.0.0.1:36657 --grpc.address localhost:1090 --p2p.laddr tcp://0.0.0.0:36656 --api.address tcp://localhost:2317 --rpc.pprof_laddr tcp://127.0.0.1:7060 --grpc-web.address localhost:1091 --home $BANKSY_HOME
